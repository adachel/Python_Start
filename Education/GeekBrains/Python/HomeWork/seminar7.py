...
def Func(arr):
    ...
    for i in range(len(arr) - 1):
        a = len(list(filter(lambda x: Vowels(x), arr[i])))
        b = len(list(filter(lambda x: Vowels(x), arr[i + 1])))
        if a == b: res = 'Парам пам-пам'
        else: res = 'Пам парам'; break
    return res
...
def print_operation_table(operation, num_rows=6, num_columns=6):
    ...
    for rows in range(1, num_rows + 1): 
        temp = []
        for colums in range(1, num_columns + 1):
            res = operation(rows, colums)
            temp.append(res)
        arr.append(temp)
    for i in range(len(arr)):   # вывод матрицы
